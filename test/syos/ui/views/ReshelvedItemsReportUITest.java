 ...
 */
@DisplayName("ReshelvedItemsReportUI Tests")
class ReshelvedItemsReportUITest {
    ...
    // Gather Report Criteria Tests @Test
    ...
    void testGatherReportCriteria_UserGoesBackFromEndDate() {
        // Arrange
        Scanner scanner = createScanner("1\n4\n2025-06-01\n\n"); // In-store + specific range + start date + empty end
                                                                 // date
        reshelvedItemsReportUI = new ReshelvedItemsReportUI(scanner);
    ...
    void testFetchReportItems_InStoreRange() {
        ...
        try (MockedConstruction<ReshelvedItemsReportService> mockedConstruction = mockConstruction(
                ReshelvedItemsReportService.class, (mock, context) -> {
                    when(mock.getReshelvedItemsForInStoreRange(any(LocalDate.class), any(LocalDate.class)))
                            .thenReturn(mockItems);
                });
                MockedStatic<ConsoleUtils> mockedUtils = mockStatic(ConsoleUtils.class)) {

            // Arrange - Create UI inside the mocked construction block
            Scanner scanner = createScanner("1\n1\n"); // In-store + Today
            reshelvedItemsReportUI = new ReshelvedItemsReportUI(scanner);
    ...
    void testFetchReportItems_OnlineRange() {
        ...
        try (MockedConstruction<ReshelvedItemsReportService> mockedConstruction = mockConstruction(
                ReshelvedItemsReportService.class, (mock, context) -> {
                    when(mock.getReshelvedItemsForOnlineRange(any(LocalDate.class), any(LocalDate.class)))
                            .thenReturn(mockItems);
                });
                MockedStatic<ConsoleUtils> mockedUtils = mockStatic(ConsoleUtils.class)) {
    ...
    void testFetchReportItems_CombinedRange() {
        ...
        try (MockedConstruction<ReshelvedItemsReportService> mockedConstruction = mockConstruction(
                ReshelvedItemsReportService.class, (mock, context) -> {
                    when(mock.getAllReshelvedItemsRange(any(LocalDate.class), any(LocalDate.class)))
                            .thenReturn(mockItems);
                });
                MockedStatic<ConsoleUtils> mockedUtils = mockStatic(ConsoleUtils.class)) {
    ...
    @Test
    @DisplayName("Debug - Simple test without output assertions")
    void testGatherReportCriteria_Debug() {
        // Arrange
        Scanner scanner = createScanner("1\n1\n"); // In-store + Today
        reshelvedItemsReportUI = new ReshelvedItemsReportUI(scanner);
        ...
        try (MockedStatic<ConsoleUtils> mockedUtils = mockStatic(ConsoleUtils.class)) {
            ...
            // Assert - Only check the return value
            assertTrue(result, "gatherReportCriteria should return true");
        }
    }
}